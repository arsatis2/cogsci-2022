...
- `relationBlue`: likelihood that blue caused bulb to switch on
- `relationOrange`: likelihood that orange caused bulb to switch on
- `relationOrangeBlue`: likelihood that both caused bulb to switch on
- `expBlue`: likelihood that agent expected blue to cause bulb to switch on
- `expOrange`: likelihood that agent expected orange to cause bulb to switch on
- `expBoth`: likelihood that agent expected both to cause bulb to switch on
...
On causal inference, participants in the emotion condition were less likely to infer that the blue box turned on the bulb ($t(127) = -3.33, p = .001$), than participants in the no-emotion condition.
```{r testing-hypotheses-on-causal-structure-blue}
t.test(relationBlue ~ condition, ecl_dat)
```

On the other hand, participants in the emotion condition were more likely to infer that the orange box turned on the bulb ($t(120.63) = 3.06, p = .003$), than participants in the no-emotion condition.
```{r testing-hypotheses-on-causal-structure-orange}
t.test(relationOrange ~ condition, ecl_dat)
```

Also, participants in the emotion condition were less likely to infer that pushing both boxes turned on the bulb ($t(100.42) = -3.51, p = .007$), than participants in the no-emotion condition.
...
Participants in the emotion condition were less likely to infer that the agent expected both boxes to turn on the bulb ($t(116.91) = -3.56, p < 0.001$) than participants in the no-emotion condition.
